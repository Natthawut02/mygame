...
class StartScreen(Screen):
    def __init__(self, **kwargs):
        ...
        buy_monster_button.bind(on_press=self.open_buy_monster)
        layout.add_widget(buy_monster_button)
    ...
    def open_buy_monster(self, instance):
...
class GameScreen(Screen):
    ...
    def save_game_state(self, save_name):
        game_state = {
            ...
            "monster_image": self.monster_image.source,
        }
    ...
    def load_game_state(self, save_name):
        ...
        try:
            with open(save_path, "r") as save_file:
                ...
                self.monster_image.source = game_state["monster_image"]
...
class BuyMonsterScreen(Screen):
    def __init__(self, **kwargs):
        ...
        background = Image(
            source="shop_background2.jpeg", allow_stretch=True, keep_ratio=False
        )
        main_layout.add_widget(background)
        ...
        self.monster_prices = {
            "Monster A": {"price": 100, "description": "A strong monster"},
            "Monster B": {"price": 150, "description": "A fast monster"},
            "Monster C": {"price": 200, "description": "A rare monster"},
            "Monster D": {"price": 250, "description": "A legendary monster"},
        }

        monsters_grid = GridLayout(
            cols=2, spacing=15, size_hint=(1, 0.7), padding=[10, 10]
        )

        for monster, details in self.monster_prices.items():
            monster_card = BoxLayout(orientation="vertical", spacing=5, padding=10)

            with monster_card.canvas.before:
                Color(0.1, 0.1, 0.1, 0.9)
                Rectangle(pos=monster_card.pos, size=monster_card.size)

            image_container = BoxLayout(size_hint=(1, 0.8), padding=[2, 2])
            with image_container.canvas.before:
                Color(1, 1, 1, 0.8)
                Rectangle(pos=image_container.pos, size=image_container.size)
            ...
            monster_image = Image(
                source=(
                    "diji.jpeg"
                    if monster == "Monster A"
                    else (
                        "jimon.jpeg"
                        if monster == "Monster B"
                        else "kiji.jpeg" if monster == "Monster C" else "hula.jpeg"
                    )
                ),
                size_hint=(1, 1),
            )
            image_container.add_widget(monster_image)
            ...
            buy_button.monster_name = monster
            buy_button.bind(on_press=self.buy_monster)

            monster_card.add_widget(image_container)
            monster_card.add_widget(monster_name)
            monster_card.add_widget(price_label)
            monster_card.add_widget(description_label)
            monster_card.add_widget(buy_button)

            monsters_grid.add_widget(monster_card)

        content_layout.add_widget(monsters_grid)
    ...
    def buy_monster(self, instance):
        monster_name = instance.monster_name
        monster_price = self.monster_prices[monster_name]["price"]
        current_gold = int(self.manager.get_screen("game").gold_label.text.split()[1])

        if current_gold >= monster_price:
            current_gold -= monster_price
            self.manager.get_screen("game").gold_label.text = f"Gold: {current_gold}"
            self.gold_amount.text = str(current_gold)
            self.manager.get_screen("game").monster_image.source = (
                "diji.jpeg"
                if monster_name == "Monster A"
                else (
                    "jimon.jpeg"
                    if monster_name == "Monster B"
                    else "kiji.jpeg" if monster_name == "Monster C" else "hula.jpeg"
                )
            )
            self.manager.get_screen("game").monster_image.reload()

            popup = Popup(
                title="Purchase Successful!",
                content=Label(
                    text=f"You bought {monster_name}!", color=(0.2, 0.8, 0.2, 1)
                ),
                size_hint=(0.6, 0.2),
                background_color=(0.9, 0.9, 0.9, 1),
            )
            ...
            try:
                purchase_sound = SoundLoader.load("shop.mp3")
                if purchase_sound:
                    purchase_sound.volume = 0.3
                    purchase_sound.play()
            except:
                print("Warning: Could not play purchase sound")
        else:
            needed_gold = monster_price - current_gold
            popup = Popup(
                title="Cannot Purchase",
                content=Label(
                    text=f"Not enough gold!\nYou need {needed_gold} more gold.",
...
class GameApp(App):
    def build(self):
        ...
        sm.add_widget(BuyMonsterScreen(name="buy_monster"))
